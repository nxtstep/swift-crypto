...
extension BrainpoolP256r1.KeyAgreement.PrivateKey {
    internal func openSSLSharedSecretFromKeyAgreement(with publicKeyShare: BrainpoolP256r1.KeyAgreement.PublicKey) throws -> SharedSecret {
        let key = try self.impl.key.keyExchange(publicKey: publicKeyShare.impl.key)
        return SharedSecret(ss: key)
    }
}
